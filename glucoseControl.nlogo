...
to setup
  ...
  set glucose-baseline 3
  set hormone-half-life 2
  set hormone-mean-life hormone-half-life / ln 2
  set meal-size 7000
  set meal-length 17
...
"" ""
PENS
"glucose" 1.0 0 -955883 true "" "plot (count glucoses)"
"insulin" 1.0 0 -11221820 true "" "plot (count insulins)"
"glucagon" 1.0 0 -2064490 true "" "plot (count glucagons)"
...
This is an agent-based model of glucose control by the liver and pancreas through the production of the hormones insulin and glucagon.

This model represents cells as square patches which are either brown (liver cells) or yellow (pancreatic cells).  These two tissues work together to regulate the levels of the signal molecules glucose (orange dots), insulin (cyan dots), and glucagon (pink dots) in the bloodstream.

The model demonstrates how various factors affect the automatic balance of blood glucose.
...
One of the most common factors that might affect this is activity level or exercise.  You can simulate changes in this by adjusting the `METABOLIC-RATE` slider.  Another common factor that influences homeostasis is eating, which can be simulated by pressing the `EAT` button.
...
`MOLECULES`:  Shows the counts of glucose, insulin, and glucagon in the bloodstream.

## THINGS TO TRY

The default sensitivity values are meant to mimic a healthy metabolism.  Try to model type 2 diabetes, where the body's ability to detect insulin is dramatically reduced, by changing the sensitivity sliders.

When you eat there is a small spike in blood glucose. Check out how it's affect is changed by different conditions!
@#$#@#$#@
default
true
0
